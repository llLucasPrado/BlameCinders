...
import com.root.game.Combate.SistemaFurtividade;
...
import com.root.game.Itens.Comida;
...
public class TCC_0_01 extends ApplicationAdapter {
    ...
    private void criarTabuleiroVisual() {
        ...
        for (int i = 0; i < Tabuleiro.LINHAS; i++) {
            for (int j = 0; j < Tabuleiro.COLUNAS; j++) {
                ...
                Cartas carta = new Cartas(frente, verso, x, y, i, j, this, fonteCarta);
    ...
    private Drawable criarDrawableCor(Color cor) {
        return new TextureRegionDrawable(new TextureRegion(TextureManager.getSolid(cor)));
    }
        ...
        popupManager.mostrarPopupMensagem("Baú encontrado!", () -> {
            ...
            if (cartaZoomAtual != null) {
                animacaoCarta.aplicarFlip(cartaZoomAtual, () -> cartaZoomAtual.setDrawable(
                    new TextureRegionDrawable(
                        new TextureRegion(
                            TextureManager.get(cartaInfo.getItemDentro().getIdentificadorVisual())
                        )
                    )
                ));
            }
